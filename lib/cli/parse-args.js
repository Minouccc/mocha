      ...
      Object.entries(config).forEach(([rawName, value]) => {
        ...
        if (name === "_") {
          ...
          if (config === defaultValues) {
            ...
            merged[name] = value;
          } else if (name === "reporter-option") {
            // `reporter-option` values are reduced downstream with
            // last-writer-wins semantics (`coerceReporterOption`), so they
            // must accumulate in priority-LOW->HIGH order for a higher-priority
            // source (e.g. CLI) to override a lower-priority one (e.g. RC
            // file). Every other array option keeps the existing prepend
            // order, which its consumers rely upon.
